...
def generate_spatial_cot(sampled_indices, masks, caption):
    ...
    if no_lesion:
        cot += f"Z={no_lesion} — target not visible on these slices, skipping annotation.\n"
...
def build_trajectory(anno, jpeg_root, render_dir, inject_fail: bool):
    ...
    user_lines = [
        ...
        f"For each slice that contains the target, {_task_desc} by calling add_bbox with the precise "
        f"2-D bounding box [x1, y1, x2, y2] in pixel coordinates. "
...
def convert(args):
    ...
    # Filter to organ + lesion + cyst expressions with enough frames
    target_cats = {'organ', 'lesion', 'cyst'}
    valid_annos = [
        a for a in annos
    ...
    print(f"Organ/lesion/cyst with >={MIN_LESION_FRAMES} frames: {len(valid_annos)}")
